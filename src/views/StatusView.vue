...
async function refresh() {
  ...
  await reservationStore.loadReservations(true); // <-- adicione true aqui
...
const enrichedLabs = computed(() => {
  ...
  return labStore.labs.map((lab) => {
    const activeRes = reservationStore.reservations.find((r) => {
      ...
      // Compara diretamente sem converter para UTC
      const rDateStr = rDate.split("T")[0];
    ...
    let status = "LIVRE";
    if (["MAINTENANCE", "MANUTENCAO"].includes(rawStatus)) status = "MANUTENCAO";
    else if (activeRes || lab.available === false || ["OCCUPIED", "OCUPADO"].includes(rawStatus))
      status = "OCUPADO";

    return { ...lab, computed: status, activeRes };
  });
});
...
<template>
  <AppLayout>
    ...
    <div class="counters">
      <div
        class="counter"
        :class="{ active: filterStatus === 'LIVRE' }"
        @click="filterStatus = 'LIVRE'"
      >
        <CheckCircle2 :size="22" color="#16a34a" />
        <div>
          <div class="c-num green">{{ totals.livre }}</div>
          <div class="c-label">Livre{{ totals.livre !== 1 ? "s" : "" }}</div>
        </div>
      </div>
      <div
        class="counter"
        :class="{ active: filterStatus === 'OCUPADO' }"
        @click="filterStatus = 'OCUPADO'"
      >
        <XCircle :size="22" color="#ef4444" />
        <div>
          <div class="c-num red">{{ totals.ocupado }}</div>
          <div class="c-label">Ocupado{{ totals.ocupado !== 1 ? "s" : "" }}</div>
        </div>
      </div>
      <div
        class="counter"
        :class="{ active: filterStatus === 'MANUTENCAO' }"
        @click="filterStatus = 'MANUTENCAO'"
      >
        <Wrench :size="22" color="#f59e0b" />
        <div>
          <div class="c-num yellow">{{ totals.manutencao }}</div>
          <div class="c-label">Manutencao</div>
        </div>
      </div>
      <div
        class="counter"
      ...
      >
        ...
        <div>
          <div class="c-num purple">{{ enrichedLabs.length }}</div>
    ...
    <div v-else class="labs-grid">
      ...
      >
        ...
        <div v-else class="info-box info-livre">Disponivel para reserva.</div>
