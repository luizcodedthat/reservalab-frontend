...
function labStatusCfg(lab) {
  ...
  if (['MAINTENANCE','MANUTENCAO'].includes(raw))
    return { icon: Wrench,       cls: 'manutencao', label: 'MANUTENÇÃO' }
...
// ─── Meus chamados — campos corrigidos ───────────────────────────────────────
const myOpenTickets = computed(() =>
  ticketStore.tickets.filter(t => {
    ...
    const isMine = t.createdByUserId === auth.user?.id ||
                   t.authorId        === auth.user?.id ||
...
// ─── labsMap para nome do lab no ticket ──────────────────────────────────────
const labsMap = computed(() => {
  const map = {}
  for (const lab of labStore.labs ?? []) map[lab.id] = lab.name  // lab.name não lab.nome
  return map
})

function getTicketLabName(t) {
  return labsMap.value[t.laboratoryId] || labsMap.value[t.labId] || `Lab ${t.laboratoryId || '-'}`
}
...
<template>
  <AppLayout campus="IF Campus">
    ...
    <div class="greeting-bar">
      <div>
        <h1 class="greeting">Olá, {{ auth.user?.firstName }}.</h1>
        <p class="greeting-sub">Veja o que está acontecendo nos laboratórios hoje.</p>
      </div>
    ...
    <div class="main-grid">
        ...
        <div v-else class="ticket-list">
          ...
          >
            ...
            <div class="ticket-body">
              <div class="ticket-title">{{ t.title || 'Sem título' }}</div>
              <!-- CORRIGIDO: usa getTicketLabName em vez de t.labId -->
              <div class="ticket-meta">{{ getTicketLabName(t) }} · {{ t.status }}</div>
            </div>
      ...
      <div class="right-col">
        <div class="card">
          ...
          <div v-else-if="labsStatus.length === 0" class="empty-msg">Nenhum laboratório encontrado.</div>
        ...
        <div class="card card-dark">
          <div class="card-title mb-12"><Megaphone :size="16" /> Comunicados</div>
          <div class="announce-list">
            <div class="announce-item">
              <div class="announce-title">Bem-vindo ao ReservaLab</div>
              <div class="announce-text">Acompanhe o status dos laboratórios e abra chamados pelo portal.</div>
            </div>
          </div>
          <button class="btn-ghost-white" @click="router.push('/chamados')">Ver chamados</button>
        </div>
      </div>
    </div>
    ...
    <div class="section-header">
      <div>
        <h2 class="section-title">Consultar Laboratórios</h2>
        <p class="section-sub">Explore as instalações e recursos disponíveis.</p>
      </div>
    ...
    <div v-if="labStore.loading" class="empty-msg">Carregando laboratórios...</div>
    <div v-else class="labs-grid">
      ...
      >
        ...
        <div class="lab-body">
          <div class="lab-name-row">
            <!-- CORRIGIDO: lab.name e lab.code -->
            <span class="lab-name">{{ lab.name }}</span>
            <span class="lab-code">{{ lab.code }}</span>
          </div>
          <p class="lab-desc">{{ lab.description || 'Laboratório do campus' }}</p>
          <div class="lab-icons">
      ...
      <div v-if="labsGrid.length === 0" class="labs-empty">Nenhum laboratório disponível.</div>
    </div>
...
.meta-label   { display:block; font-size:10px; color:#94a3b8; text-transform:uppercase; letter-spacing:.5px; margin-bottom:4px; }
...
.ticket-row   { display:flex; align-items:center; gap:10px; padding:10px; border-radius:8px; cursor:pointer; transition:background .1s; border:1px solid #f8fafc; }
.ticket-row:hover { background:#f8fafc; }
.ticket-dot   { width:9px; height:9px; border-radius:50%; flex-shrink:0; }
.dot-open     { background:#ef4444; } .dot-progress { background:#f59e0b; } .dot-done { background:#16a34a; }
.ticket-body  { flex:1; }
...
.btn-outline { display:flex; align-items:center; justify-content:center; gap:6px; width:100%; padding:8px; border:1px dashed #d1d5db; background:none; border-radius:8px; font-size:13px; color:#64748b; cursor:pointer; transition:all .15s; }
.btn-outline:hover { border-color:#16a34a; color:#16a34a; background:#f0fdf4; }
.right-col   { display:flex; flex-direction:column; gap:12px; }
.status-list { display:flex; flex-direction:column; gap:10px; }
.status-row  { display:flex; align-items:center; gap:8px; cursor:pointer; padding:4px 6px; border-radius:6px; transition:background .1s; }
...
.icon-livre      { color:#16a34a; } .icon-ocupado { color:#ef4444; } .icon-manutencao { color:#f59e0b; }
...
.lab-card   { background:#fff; border:1px solid #e8ecf0; border-radius:12px; overflow:hidden; text-decoration:none; display:block; transition:box-shadow .15s; }
.lab-card:hover { box-shadow:0 4px 14px rgba(0,0,0,.09); }
