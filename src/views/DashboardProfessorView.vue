...
onMounted(async () => {
  ...
  // Sempre força reload das reservas para mostrar as recém criadas
  await reservationStore.loadReservations(true)
})

// ─── Minhas reservas (compatível com todos os campos possíveis) ───────────────
const myReservations = computed(() =>
  reservationStore.reservations.filter(r => {
    const uid = auth.user?.id
    return (
      r.requestedByUserId === uid ||
      r.professorId       === uid ||
      r.userId            === uid
    )
  })
...
const activeReservations = computed(() =>
  myReservations.value.filter(r => {
    ...
    return ['APPROVED','CONFIRMADA','PENDING','PENDENTE'].includes(st)
  })
)
...
const nextReservation = computed(() => {
  ...
  return [...activeReservations.value]
    ...
    .sort((a, b) => {
      const da = new Date((a.reservationDate || a.date || ''))
      const db = new Date((b.reservationDate || b.date || ''))
      return da - db
    })[0] ?? null
})
...
const myTickets = computed(() =>
  ticketStore.tickets.filter(t => {
    const uid = auth.user?.id
    return t.authorId === uid || t.requestedByUserId === uid || t.userId === uid
  })
)

const myOpenTickets = computed(() =>
  myTickets.value.filter(t =>
...
// ─── Agenda semanal ───────────────────────────────────────────────────────────
const SHOW_DAYS = ['SEGUNDA','TERCA','QUARTA','QUINTA','SEXTA']
...
const recentActivities = computed(() => {
  ...
  myReservations.value.slice(0, 2).forEach(r => {
    items.push({
      ...
      desc:  r.labName || r.labId || 'Laboratorio',
...
function activityColor(type) {
  return type === 'reserva' ? '#16a34a' : '#ef4444'
}
...
function formatRelative(ts) {
  ...
  if (h < 1)  return 'ha poucos minutos'
  ...
  <AppLayout>
    ...
    <div class="top-grid">
      ...
      <div class="card stat-card">
        ...
        <div v-if="nextReservation" class="stat-sub">
          Proxima: {{ nextReservation.labName || nextReservation.labId || 'Lab' }}
          &bull; {{ formatTime(nextReservation) }}
        </div>
        <div v-else class="stat-sub">Nenhuma reserva proxima</div>
      </div>
      ...
      <div class="card stat-card">
        ...
        <div class="stat-sub">{{ myResolvedThisMonth }} concluidos este mes</div>
      </div>
      ...
      <div class="card">
        ...
        <div v-if="recentActivities.length === 0" class="empty-msg">
      ...
      <div v-else class="week-grid">
        <div v-for="d in weekAgenda" :key="d.day" class="week-col">
          ...
          >
            ...
            <div class="week-lab">{{ d.res.labName || d.res.labId || 'Lab' }}</div>
    ...
    <div class="bottom-grid">
      ...
      <div class="tip-card">
        ...
        <p class="tip-text">
          Acompanhe seus chamados e reserve laboratorios com antecedencia para garantir disponibilidade.
        </p>
        <button class="btn-white" @click="router.push('/reservas')">
          Ver minhas reservas
        </button>
      </div>
      ...
      <div class="card">
        ...
        <div v-if="myTickets.length === 0" class="empty-msg">
          ...
          >
            ...
            <div class="ticket-body">
              ...
              <span class="priority-label" :class="priorityClass(t.priority)">
                &bull; {{ (t.priority || 'MEDIA').toUpperCase() }} PRIORIDADE
              </span>
            </div>
          </div>
        </div>
...
.week-day   { font-size: 10.5px; font-weight: 600; color: #94a3b8; letter-spacing: 0.4px; display: flex; flex-direction: column; gap: 1px; }
.week-date  { font-size: 10px; font-weight: 400; color: #cbd5e1; }
.week-block { background: #f0fdf4; border-left: 3px solid #16a34a; border-radius: 7px; padding: 10px; cursor: pointer; transition: background 0.1s; }
...
.tip-tag   { display: inline-block; background: rgba(255,255,255,0.15); font-size: 10px; padding: 3px 9px; border-radius: 4px; width: fit-content; }
.tip-title { font-size: 20px; font-weight: 700; margin: 0; line-height: 1.3; }
.tip-text  { font-size: 13px; opacity: 0.85; line-height: 1.5; margin: 0; }
...
.ticket-row  { display: flex; gap: 10px; align-items: flex-start; cursor: pointer; padding: 8px; border-radius: 8px; transition: background 0.1s; }
