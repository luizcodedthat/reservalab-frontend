...
function ticketIcon(t) {
  ...
  if (title.includes('insumo') || title.includes('quimica') || title.includes('material')) return Leaf
  return Wrench
}
    ...
    <div class="bottom-grid">
      ...
      <div class="card">
        ...
        <div v-else class="ticket-list">
          ...
          <div v-if="recentTickets.length === 0" class="empty-row-txt">
...
.page-title  { font-size: 26px; font-weight: 800; color: #0f172a; margin: 0 0 3px; }
.page-sub    { font-size: 13px; color: #64748b; margin: 0; }
.header-actions { display: flex; gap: 10px; }
.btn-primary  { display: flex; align-items: center; gap: 6px; background: #16a34a; color: #fff; border: none; padding: 9px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; }
.btn-primary:hover { background: #15803d; }
.btn-secondary{ display: flex; align-items: center; gap: 6px; background: #fff; color: #374151; border: 1px solid #d1d5db; padding: 9px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; }
.btn-secondary:hover { background: #f8fafc; }

.stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 12px; }
.stat-card  { background: #fff; border: 1px solid #e8ecf0; border-radius: 12px; padding: 16px; cursor: pointer; transition: box-shadow 0.15s; }
...
.stat-icon  { width: 40px; height: 40px; border-radius: 9px; display: flex; align-items: center; justify-content: center; }
.bg-green { background: #f0fdf4; }
.bg-red   { background: #fef2f2; }
.bg-purple{ background: #f5f3ff; }
.bg-blue  { background: #eff6ff; }
.stat-badge { font-size: 10px; font-weight: 600; padding: 2px 7px; border-radius: 8px; }
...
.stat-num   { font-size: 33px; font-weight: 800; color: #0f172a; line-height: 1; }

.mid-grid   { display: grid; grid-template-columns: 1fr 290px; gap: 12px; margin-bottom: 12px; }
...
.chart-sub  { font-size: 12px; color: #94a3b8; margin-top: 3px; }

.bar-chart  { display: flex; gap: 10px; height: 160px; align-items: flex-end; }
.bar-col    { flex: 1; display: flex; flex-direction: column; align-items: center; height: 100%; }
.bar-pct    { font-size: 10px; color: #94a3b8; margin-bottom: 4px; }
.bar-track  { flex: 1; width: 100%; display: flex; align-items: flex-end; }
.bar-fill   { width: 100%; background: linear-gradient(180deg, #16a34a 0%, #4ade80 100%); border-radius: 5px 5px 0 0; min-height: 4px; transition: height 0.4s ease; }
...
.link-btn    { background: none; border: none; color: #16a34a; font-size: 13px; font-weight: 500; cursor: pointer; }

.res-table  { width: 100%; border-collapse: collapse; }
.res-table th { font-size: 10px; font-weight: 600; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; padding: 5px 0 8px; text-align: left; border-bottom: 1px solid #f1f5f9; }
.res-table td { padding: 9px 0; font-size: 13px; color: #374151; border-bottom: 1px solid #f8fafc; vertical-align: middle; }
.res-table tr:last-child td { border-bottom: none; }
.prof-cell  { display: flex; align-items: center; gap: 8px; }
.prof-av    { width: 28px; height: 28px; background: #e2e8f0; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: #475569; flex-shrink: 0; }
.status-pill{ font-size: 10px; font-weight: 700; padding: 2px 7px; border-radius: 4px; }
...
.ticket-row  { display: flex; gap: 10px; align-items: flex-start; cursor: pointer; padding: 7px; border-radius: 8px; transition: background 0.1s; }
.ticket-row:hover { background: #f8fafc; }
.ticket-icon { width: 34px; height: 34px; background: #f1f5f9; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.ticket-body { flex: 1; }
.ticket-top  { display: flex; justify-content: space-between; gap: 6px; }
.ticket-title{ font-size: 13px; font-weight: 600; color: #0f172a; }
.ticket-time { font-size: 11px; color: #94a3b8; white-space: nowrap; }
.ticket-desc { font-size: 12px; color: #64748b; margin: 2px 0 4px; }
...
.loading-msg   { text-align: center; color: #94a3b8; font-size: 13px; padding: 20px; }
.empty-row     { text-align: center; color: #94a3b8; padding: 20px !important; font-size: 13px; }
.empty-row-txt { text-align: center; color: #94a3b8; font-size: 13px; padding: 16px 0; }
</style>
