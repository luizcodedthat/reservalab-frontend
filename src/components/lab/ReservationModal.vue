...
async function handleSubmit() {
  ...
  if (!form.value.labId) {
    errorMessage.value = "Selecione o laboratório.";
    return;
  }
  if (!form.value.date) {
    errorMessage.value = "Informe a data da reserva.";
    return;
  }
  if (!form.value.startTime) {
    errorMessage.value = "Informe o horário de início.";
    return;
  }
  if (!form.value.endTime) {
    errorMessage.value = "Informe o horário de término.";
    return;
  }
  ...
  try {
    ...
    errorMessage.value =
      err?.response?.data?.message || err.message || "Não foi possível confirmar a reserva.";
  } finally {
              ...
              <Transition name="ai-panel">
                <div v-if="isAiPanelOpen" class="ai-panel">
                  <p class="ai-panel__hint">
                    Descreva sua necessidade e a IA sugerirá o laboratório mais adequado para o
                    período escolhido.
                  </p>
...
.btn--submit {
  ...
  color: #ffffff;
  box-shadow: 0 4px 12px rgba(0, 107, 31, 0.2);
}
...
.lab-label-row { display: flex; align-items: center; justify-content: space-between; }
.ai-toggle-btn {
...
.ai-panel {
  background: #f0f7f1; border: 1px solid #c2ddc8;
  border-radius: 0.5rem; padding: 0.875rem 1rem;
  display: flex; flex-direction: column; gap: 0.625rem;
}
.ai-panel__hint { font-size: 0.75rem; color: #3f4a3c; line-height: 1.4; }
.ai-input-row { display: flex; gap: 0.5rem; align-items: flex-start; }
.ai-textarea {
  flex: 1; background: #ffffff; border: 1px solid #c2ddc8;
  border-radius: 0.375rem; padding: 0.5rem 0.75rem;
  font-size: 0.8125rem; font-family: "Inter", sans-serif;
  color: #1a1c1c; resize: none; outline: none; transition: box-shadow 0.2s; line-height: 1.4;
}
...
.ai-send-btn {
  ...
  background: #006b1f; color: #fff; border: none; border-radius: 0.375rem;
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; transition: opacity 0.15s, transform 0.1s; margin-top: 0.125rem;
}
...
.ai-hint-warn { font-size: 0.7rem; color: #8a6800; font-weight: 500; }
.ai-error { font-size: 0.75rem; color: #ba1a1a; font-weight: 500; }
.ai-reason {
  display: flex; align-items: flex-start; gap: 0.375rem;
  font-size: 0.75rem; color: #1a4225; line-height: 1.45;
  background: #d6f0dc; border-radius: 0.375rem; padding: 0.5rem 0.625rem;
}
.ai-reason__icon { flex-shrink: 0; margin-top: 0.1rem; color: #006b1f; }
.ai-panel-enter-active, .ai-panel-leave-active { transition: opacity 0.2s ease, transform 0.2s ease; }
.ai-panel-enter-from, .ai-panel-leave-to { opacity: 0; transform: translateY(-6px); }
</style>
