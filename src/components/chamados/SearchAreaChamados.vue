...
const emit = defineEmits(['update:search', 'update:status', 'update:date'])
    ...
    <div class="filter-container">
      ...
      <Transition name="dropdown">
        <div v-if="showFilters" class="filter-dropdown">
          ...
          <p class="filter-dropdown__title">Data</p>
          <input
          ...
          <div class="filter-actions">
            <button class="filter-clear-btn" @click="clearFilters">Limpar</button>
            <button class="filter-apply-btn" @click="applyFilters">Aplicar</button>
          </div>
...
.filter-divider { height: 1px; background: #f1f5f9; margin: 1rem 0; }
.filter-date {
...
.filter-date:focus { box-shadow: 0 0 0 2px #006b1f; border-color: transparent; }
.filter-actions {
  display: flex; gap: 0.5rem; margin-top: 1rem;
}
.filter-clear-btn {
  flex: 1; padding: 0.625rem;
  background: #f3f3f4; color: #3f4a3c; border: none;
  border-radius: 0.375rem; font-size: 0.75rem; font-weight: 700;
  text-transform: uppercase; letter-spacing: 0.05em; cursor: pointer;
}
.filter-clear-btn:hover { background: #e2e2e2; }
.filter-apply-btn {
  flex: 1; padding: 0.625rem;
